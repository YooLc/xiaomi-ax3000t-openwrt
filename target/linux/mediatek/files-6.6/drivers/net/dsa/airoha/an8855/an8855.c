			...
			struct netlink_ext_ack *extack)
{
	...
	dsa_switch_for_each_user_port(other_dp, ds) {
		...
		if (priv->ports[other_port].enable)
			an8855_set(priv, AN8855_PORTMATRIX_P(other_port),
					   PORTMATRIX_MATRIX(BIT(port)));
		priv->ports[other_port].pm |= PORTMATRIX_MATRIX(BIT(port));
			...
			struct dsa_bridge bridge)
{
	...
	dsa_switch_for_each_user_port(other_dp, ds) {
		...
		if (priv->ports[other_port].enable)
			an8855_clear(priv, AN8855_PORTMATRIX_P(other_port),
				     PORTMATRIX_MATRIX(BIT(port)));
		priv->ports[other_port].pm &= PORTMATRIX_MATRIX(BIT(port));
	}
...
static int
an8855_port_vlan_filtering(struct dsa_switch *ds, int port, bool vlan_filtering, struct netlink_ext_ack *extack)
{
	if (vlan_filtering) {
			 ...
			 struct netlink_ext_ack *extack)
{
	...
	an8855_hw_vlan_update(priv, vlan->vid, &new_entry,
					an8855_hw_vlan_add);
